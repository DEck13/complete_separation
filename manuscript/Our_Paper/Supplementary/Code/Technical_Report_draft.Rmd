...
```{r}
find_cutoff <- function(y,pred){
  optimal_cutoff <- max <- -1
  for(i in c(seq(0,1,by=0.01),0.5)){
    acc <- mean(as.numeric(pred>=i) == y)
    if(acc >= max){
      optimal_cutoff = i
      max = acc
    }
...
```{r, eval=FALSE}
find_cutoff(y,total_mat_glmdr_test)
find_cutoff(y,total_mat_bayes_test)
find_cutoff(y,total_mat_brglm_test)
find_cutoff(y,total_mat_linear_test)
```
```{r}
...
```{r}
mean(as.numeric(total_mat_glmdr_test >= 0.545) == y)
mean(as.numeric(total_mat_bayes_test >= 0.5) == y)
mean(as.numeric(total_mat_brglm_test >= 0.5) == y)
mean(as.numeric(total_mat_linear_test >= 0.5) == y)
```
...
```{r}
find_cutoff(dat$Kernel.color,glmdr_pred)
find_cutoff(dat$Kernel.color,bayes_pred$fit)
find_cutoff(dat$Kernel.color,logistif_pred)
find_cutoff(dat$Kernel.color,lm_pred$fit)
```
...
```{r, eval=FALSE}
find_cutoff(dat$Kernel.color,total_mat_glmdr_test)
find_cutoff(dat$Kernel.color,total_mat_bayes_test)
find_cutoff(dat$Kernel.color,total_mat_logistf_test)
find_cutoff(dat$Kernel.color,total_mat_linear_test)
```

```{r}
mean(as.numeric(total_mat_glmdr_test >= 0.57) == dat$Kernel.color)
mean(as.numeric(total_mat_bayes_test >= 0.62) == dat$Kernel.color)
mean(as.numeric(total_mat_logistf_test >= 0.85) == dat$Kernel.color)
mean(as.numeric(total_mat_linear_test >= 0.62) == dat$Kernel.color)
```
