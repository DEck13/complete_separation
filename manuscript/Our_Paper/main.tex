...
In the linear model, we can easily obtain this expected value from $\beta$ since $\text{E}(Y|X=x) = x^{T}\beta$. Plugging in $\hat{\beta}$ produces the MLE for this expectation $\widehat{\text{E}}(Y|X=x) = x^{T}\hat\beta$ with $x$ fixed. On the other hand, in the logistic model, $\text{E}(Y|X=x_{i}) = \text{Pr}(Y = 1 | X=x_{i}) = p_i$ while $\log(\frac{p_i}{1-p_i}) = x_i^T\beta$. Therefore, $\beta$ does not offer easy interpretation about changes in the expected response as the covariates change.
%we cannot obtain the expected value of response variable in the same way as we did in the linear model.

To get the expected value from the logistic model, we can consider the mean-value parameterization. The mean-value parameterization starts with estimating the logistic model's coefficients $\beta$ by solving the \eqref{logit_Score}. Then, $x_{i}^{T}\hat{\beta}$ corresponds to the a estimated log-odds ratio of each observation which is linked to the mean-value parameters through the change-of-parameter mappings. That is, we can obtain the mean-value parameters using the inverse logit function, $\hat{\mu} = \text{logit}^{-1}(x_{i}^{T}\hat{\beta})$. The advantage of this parameterization is now our parameters of interest shifts to the mean-value parameters from the coefficients of the model. Consequently, we can provide a more informative and intuitive inference. For example, we can tell the expected probability of success at particular $x$ which is what we desire from the statistical model (without mean-value parameterization, our interpretation on model is that as one unit of explanatory variable increases the expected change in log odds ratio of conditional probability of success is $\hat{\beta}$).
...
\centering
    ...
    & \multicolumn{1}{l}{\textbf{glmdr}} & \multicolumn{1}{l}{bayesglm} & \multicolumn{1}{l}{brglm2 / logistf} & \multicolumn{1}{l}{linear} & \multicolumn{1}{l}{\textbf{glmdr} } & \multicolumn{1}{l}{bayesglm} & \multicolumn{1}{l}{brglm2 / logistf} & \multicolumn{1}{l}{linear}  \\
    \hline
...
\begin{table}[!ht]
\caption{Prediction results for all examples.}
\caption*{\footnotesize \textit{glmdr denotes Generalized Linear Model Done Right \sqcitep{glmdr_package}, bayesglm denotes Generalized Linear Model with Student-t prior distribution \sqcitep{Gelman2008}, brglm denotes Bias Reduction in Generalized Linear Models \sqcitep{Kosmidis2009}, logistf denotes Logistic model with Firth's modified score function \sqcitep{Heinze2002}, and linear denotes the multiple linear model using ordinary least squares.}}\label{Tab:prediction}
\centering
    \resizebox{\columnwidth}{!}{
    \begin{tabular}{lcccccccc}
    & \multicolumn{4}{c}{out-of-sample accuracy} &\multicolumn{4}{c}{average length of prediction intervals}\\ 
    & \multicolumn{1}{l}{\textbf{glmdr}} & \multicolumn{1}{l}{bayesglm} & \multicolumn{1}{l}{brglm2 / logistf} & \multicolumn{1}{l}{linear} & \multicolumn{1}{l}{\textbf{glmdr} } & \multicolumn{1}{l}{bayesglm} & \multicolumn{1}{l}{brglm2 / logistf} & \multicolumn{1}{l}{linear}  \\
    \hline
