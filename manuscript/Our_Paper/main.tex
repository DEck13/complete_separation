...
\begin{figure}[!t]
  ...
  \begin{minipage}[b]{0.45\textwidth}
    ...
    \caption{Example of Complete Separation in Logistic Model. \textbf{Suyoung: make this one figure with two panels}}\label{Fig:Agresti_CS}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.45\textwidth}
    \includegraphics[width=\textwidth]{Figures/Agresti_Complete_Separation2.png}
    \caption{Log Likelihood Values of Logistic Model at each iteration. \textbf{Suyoung: make this one figure with two panels}}\label{Fig:Agresti_loglik}
  \end{minipage}
\end{figure}
...
Prediction in $\texttt{glmdr}$ framework is different from that of the traditional statistical model because we estimate the mean value parameters rather than coefficient. Therefore, we firstly find the prediction intervals given new data point then take an average of them for predicted value.
...
On the other hands, we can see all logistic models in Section 3.2 and 3.3 perform similarly despite of different approaches and techniques. $\texttt{glmdr}$ solves the separation problem within the original model settings unlike other methods. Also, it estimates the probability of success rather than coefficients by finding the MLE in the Barndorff-Nielsen completion based on approximate null eigenvectors of the Fisher information matrix. The $\texttt{bayesglm}$ uses a Bayesian approach which scales the data first and then placing Cauchy distribution as a prior distribution on the coefficients and $\texttt{logistf}$ (similar to $\texttt{brglm2}$) modifies the score function to produce finite coefficients when MLE does not exist or lies on infinity.
