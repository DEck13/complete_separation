...
\centering
    ...
    \begin{tabular}{ll|ccccc}
 &  & Complete Separation & Quasi Separation & Quadratic & Endometrial & Maize \\ \hline
accuracy & glmdr & 87.5 \% & 90 \% & 93.33 \% & 87.34 \% & 86.04 \% \\
...
Considering all aspects, linear model performs well despite of the binary response. It shows comparable or better out-of-sample accuracy, small prediction intervals and lowest computational cost. Among logistic models, all three methods perform comparably. However, there are several notable differences. $\texttt{glmdr}$ provides the smallest lengths of prediction intervals except in the quasi separation example. It also shows better performance in quasi separation datasets (i.e., quasi separation and endometrial examples). But, it may not be scalable to the large datasets due to relatively high computational cost. $\texttt{bayesglm}$ presents the most rounded performance with fairly low computational cost. $\texttt{brglm}$ achieves the highest out-of-sample accuracy in quadratic example, but $\texttt{brglm}$ fails to converge in maize example and alternative method, $\texttt{logistf}$, is very costly.
