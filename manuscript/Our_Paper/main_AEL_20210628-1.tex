...
\textcolor{red}{
Paragraph 1: Begin with a big-picture problem, and narrow things down to how binary response variabless are critical for solving the big-picture problem.
\begin{itemize}{
		\item Agronomically important binary traits are important
		\item Give some examples of specific binary traits, and why they are important
		\item Describe how they can be included in quantitative genetics analysis to answer important research questions
		\item Describe how they are typically treated as quantitative traits and why this is bad
		\item Describe what could happen if they were anlayzed in proper statistical models for binary traits
}\end{itemize}
}
}
...
\textcolor{red}{
Paragraph 2: Describe the problems of complete and quasi-complete separation, and how it can hinder efforts to make reaonable inferences on binary response variables.
}
...
\textcolor{red}{
Paragraph 4: Begin by explicilty stating what the purpose of the research presented in this paper is. After that, describe the objectives of the study. It would be a great idea to provide an overarching hypothesis, and what you would expect/predict ot see in your results if that hypothesis were true.
} 
...
\noindent\textbf{Complete separation}: We first analyzed the \cite{agresti_categorical_2013} example discussed in Section~\ref{sec:complete_separation}. \textcolor{red}{Briefly, these data include (give the reader enough informaitn to understand what the data are and why they are important. Do not assume that the reader knows what any acronmyms mean. Also, one rule of thumb I use is that a paragraph must have a minimum of three sentences.)} \\ 

\noindent\textbf{Quasi-complete separation}: 
 We analyze the \cite{agresti_categorical_2013} example with two points added, a success and a failure at $z = 50$ \textcolor{red}{Is the reader going to know what $z$ is?}. \\


\noindent\textbf{Quadratic logistic regression model}: This example comes from Section 2.2 of Geyer [\citeyear{Geyer2009}]. In this example $y_i =1$ for $ 12 < z_i < 24 $ and $y_i = 0$  \textcolor{red}{Is the reader going to know what $y_i$ is?}, otherwise. In this case, maximum likelihood estimate (MLE) does not exist when we fit a quadratic logistic model using $\texttt{glm}$, and it complains that the algorithm did not converge. We demonstrate how to compute the one-sided confidence intervals for mean-value parameters for this example in the supplementary material. \\

\noindent\textbf{Endometrial Cancer Study}: \cite{Heinze2002} investigated the endometrial data set (n = 79), which was originally provided by Dr. Asseryanis from the Vienna University Medical School. The main purpose of this study was to describe histology of cases (HG) in terms of three risk factors: neovasculation (NV), endometrium height (EH) and pulsatility index of arteria uterina (PI). \textcolor{red}{Try not to begin sentences with numbers or acronymns} 30 patients was classified grading 0-II for histology (HG = 1) and 49 patients for grading III-IV (HG = 0). There are 13 patients who has neovasculization (NV = 1) and absent for 66 patients (NV = 0). Pulsatility index (PI) ranges from 0 to 49 with mean of 17.38 and median of 16.00, and endometirum height (EH) ranges from 0.27 to 3.61 with mean of 1.662 and median of 1.640. In this example, we observe the quasi-complete separation in NV.  \\

\noindent\textbf{Maize data}: To predict the kernel color of maize, we used the data set from \cite{Romay2013}  that consists of 2,815 maize lines. The binary response variable we considered was the kernel color, where 1 indicated non-white kernel color and 0 indicated white kernel color. We fitted various models with kernel color as the response variable and 24 DNA markers surroudning the \textit{psy1} gene. Each marker has value from 0 to 1, \textcolor{red}{where 0 represents the reference B73 RefGen$_$v2 allele, and 1 represents the alternative allele, as described in (REFERENCE)}. In the final data set, 309 lines had a white kernel and 1,238 had non-whyite kernel color. A subset of these maize lines were subdivided into six subpopulaitons, namely 115 non-stiff stalk, 54 popcorn, 120 stiff stalk, 116 sweet corn, 159 tropical; while the remaining 983 lines were unclassified into any subpopulations. In this example, there is no separation issues when we use single marker for explanatory variable. However, we have a separation issue for saturated model\textcolor{red}{Does this mean the the inclusion of all 24 markers as explanatory variables? If yes, then I suggest explicilty stating this.} . In the later part, we mainly focus on this example.
